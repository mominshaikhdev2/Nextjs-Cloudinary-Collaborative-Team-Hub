...
const getActionItems = async (req, res) => {
  ...
  if (view === 'kanban') {
    ...
    actionItems.forEach((item) => columns[item.status].push(item));
...
const createActionItem = async (req, res) => {
  ...
  await logActivity({
    ...
    workspaceId,
    userId: req.user.id,
    metadata: { title, status: item.status },
  });
  ...
  try {
    getIO().to(`workspace:${workspaceId}`).emit('actionItem:created', item);
  } catch (_) {}
...
const reorderActionItems = async (req, res) => {
  ...
  try {
    getIO().to(`workspace:${workspaceId}`).emit('actionItems:reordered', updated);
  } catch (_) {}

  res.json({ message: 'Reordered', items: updated });
};

module.exports = { getActionItems, createActionItem, updateActionItem, deleteActionItem, reorderActionItems };