...
export default function InvitationPage() {
  ...
  useEffect(() => {
    api.get(`/api/workspaces/invitations/${token}/accept`)
  ...
  const accept = async () => {
    if (!isAuthenticated) {
      // Store token, redirect to register
      sessionStorage.setItem('pendingInvite', token);
      router.push(`/register?invite=${token}`);
      return;
    }

    // If logged in but wrong email, warn
    if (invitation.email !== user?.email) {
      toast.error(`This invitation is for ${invitation.email}. Please log in with that account.`);
      return;
    }

    setAccepting(true);
    try {
      // The backend accepts once user is logged in — we just need to upsert membership
      // For now redirect to the workspace
    ...
    return (
      <div className="min-h-screen bg-zinc-950 flex items-center justify-center">
        <div className="card p-8 text-center max-w-sm w-full mx-4">
          ...
          <h2 className="font-display font-bold text-white mb-2">Invalid Invitation</h2>
  ...
  return (
    <div className="min-h-screen bg-zinc-950 flex items-center justify-center">
      <div className="card p-8 text-center max-w-sm w-full mx-4 animate-scale-in">
        ...
        <h2 className="font-display font-bold text-white text-xl mb-2">You're invited!</h2>
        <p className="text-zinc-400 text-sm mb-1">
          You've been invited to join as <span className="text-violet-400 font-medium">{invitation.role}</span>
        </p>
        <p className="text-zinc-500 text-xs mb-8">
          Invitation for <span className="text-zinc-300">{invitation.email}</span>
        </p>

        <button onClick={accept} disabled={accepting} className="btn-primary w-full justify-center py-2.5 mb-3">
