...
export default function LoginForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Get the error message from the backend response[cite: 3]
      let msg = err.response?.data?.error || 'Login failed';

      // 1. CLEANUP: If the message contains technical jargon or is too long, mask it[cite: 3]
      const isSystemError = 
        msg.toLowerCase().includes('prisma') || 
        msg.toLowerCase().includes('invocation') || 
        msg.includes(':\\') || // Detects Windows file paths like in your screenshot
        msg.length > 100;
      ...
      // 2. STRICT FILTERING: Only show errors under fields if they are simple validation strings[cite: 3]
      if (!isSystemError) {
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <button
          type="button"
          onClick={() => setForm({ email: 'alice@demo.com', password: 'password123' })}
