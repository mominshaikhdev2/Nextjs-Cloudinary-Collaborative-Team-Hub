...
export default function AnnouncementsPage() {
  ...
  useEffect(() => {
    ...
    if (socket) {
      socket.on('announcement:created', (a) => setAnnouncements((prev) => [a, ...prev]));
      socket.on('announcement:updated', (a) => setAnnouncements((prev) => prev.map((x) => x.id === a.id ? a : x)));
      socket.on('announcement:deleted', ({ id }) => setAnnouncements((prev) => prev.filter((x) => x.id !== id)));
      socket.on('announcement:reaction', ({ announcementId, reactions }) => {
        setAnnouncements((prev) => prev.map((a) => a.id === announcementId ? { ...a, reactions } : a));
      });
    ...
    return () => {
      ...
      ['announcement:created', 'announcement:updated', 'announcement:deleted',
       'announcement:reaction', 'announcement:comment'].forEach((e) => s?.off(e));
    };
  }, [workspaceId]);
  ...
  const handleReact = async (id, emoji) => {
    try {
      const { data } = await announcementApi.addReaction(workspaceId, id, { emoji });
    ...
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-500 text-sm mt-1">Team-wide updates and news</p>
        </div>
        {currentRole === 'ADMIN' && (
      ...
      {loading && announcements.length === 0 ? (
        ...
        <EmptyState
          ...
          description={currentRole === 'ADMIN' ? 'Post the first announcement to your team' : 'Admins will post announcements here'}
          action={currentRole === 'ADMIN' && (
            <button onClick={() => setShowForm(true)} className="btn-primary">Post announcement</button>
          )}
        />
      ) : (
        <div className="space-y-4">
          ...
          {hasMore && (
            <button onClick={() => fetchAnnouncements()} className="btn-secondary w-full justify-center" disabled={loading}>
