...
export default function NotificationBell() {
  ...
  // 1. Maintain stable reference for the fetch function[cite: 3]
  ...
  // 2. useEffect with async wrapper to avoid synchronous setState warnings[cite: 3]
  useEffect(() => {
    let isMounted = true;

    const initialize = async () => {
      if (isMounted) {
        // Wrapping in an async function prevents the 'set-state-in-effect' warning[cite: 3]
  ...
  // Click-away listener for the dropdown[cite: 3]
  useEffect(() => {
    const handleClick = (e) => { 
              ...
              notifications.map((n) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-zinc-300 leading-snug">{n.content}</p>
