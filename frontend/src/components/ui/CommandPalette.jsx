...
const staticCommands = (
  ...
  const workspaceActions = currentWsId
    ? [
        nav(
          `/workspaces/${currentWsId}/goals/new`,
          "New Goal",
          "◎",
          "Quick Actions",
        ),
        nav(
          `/workspaces/${currentWsId}/action-items`,
          "Open Kanban Board",
          "◻",
          "Quick Actions",
        ),
        nav(
          `/workspaces/${currentWsId}/announcements`,
          "View Announcements",
          "◉",
          "Quick Actions",
        ),
        nav(
          `/workspaces/${currentWsId}/analytics`,
          "Open Analytics",
          "◇",
          "Quick Actions",
        ),
  ...
  return [
    ...
    },
    {
      ...
      action: () =>
        toast(
          "Shortcuts: ⌘K palette · G then D dashboard · G then G goals · T toggle theme",
          { duration: 5000 },
        ),
...
export default function CommandPalette({ isOpen, onClose }) {
  ...
  const currentWsId =
    currentWorkspace?.id || pathname.match(/\/workspaces\/([^/]+)/)?.[1];
        ...
        <div ref={listRef} className="overflow-y-auto max-h-[400px] py-2">
          {flat.length === 0 ? (
            ...
            >
              ...
              <span style={{ color: "var(--th-text-1)" }}>{query}</span>"
            </div>
          ) : (
                  ...
                  return (
                    ...
                    >
                      {cmd.iconDot ? (
                        ...
                        <span
                          className="w-5 h-5 flex items-center justify-center text-sm flex-shrink-0"
                          style={{
                            color: isSelected ? "#a78bfa" : "var(--th-text-4)",
                          }}
                        >
                          {cmd.icon}
                        </span>
