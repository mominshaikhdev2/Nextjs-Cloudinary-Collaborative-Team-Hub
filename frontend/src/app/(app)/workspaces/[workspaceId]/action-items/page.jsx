...
export default function ActionItemsPage() {
  ...
  const fetchItems = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await actionItemApi.list(workspaceId, {
        view: "kanban",
      });
    ...
    if (socket) {
      ...
      socket.on("actionItem:deleted", ({ id }) =>
        setColumns((prev) => {
          const next = { ...prev };
          Object.keys(next).forEach((col) => {
            next[col] = next[col].filter((i) => i.id !== id);
          });
          return next;
        }),
      );
  ...
  const handleCreate = async (formData) => {
    ...
    try {
      ...
      setColumns((prev) => {
        const next = { ...prev };
        Object.keys(next).forEach((col) => {
          next[col] = next[col].filter((i) => i.id !== tid);
        });
    ...
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="card p-4 space-y-3 animate-pulse min-h-[200px]"
            >
              <div className="h-3 w-1/2 th-surface-2 rounded" />
              {[1, 2].map((j) => (
                <div key={j} className="h-20 th-surface-2 rounded-lg" />
              ))}
            </div>
          ))}
        </div>
      ...
      >
        <ActionItemForm
          ...
          onSuccess={(formData) =>
            editItem
              ? handleUpdate(editItem.id, formData)
