...
export default function GoalDetailPage() {
  ...
  const postUpdate = async (e) => {
    ...
    setPosting(true);
    try {
      const { data } = await goalApi.addUpdate(workspaceId, goalId, {
        content: updateText,
      });
  ...
  const addMilestone = async (e) => {
    ...
    try {
      const { data } = await milestoneApi.create(workspaceId, goalId, {
        title: newMilestone,
      });
      setGoal((prev) => ({
        ...prev,
      ...
      const status =
        progress === 100
          ...
          : progress > 0
            ? "IN_PROGRESS"
            : "PENDING";
      const { data } = await milestoneApi.update(
        workspaceId,
        goalId,
        milestoneId,
        { progress, status },
                ...
                <div key={m.id} className="flex items-center gap-4">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1.5">
                      <span
                        className={`text-sm font-medium ${m.status === "COMPLETED" ? "text-zinc-500 line-through" : "text-zinc-200"}`}
                      >
                        {m.title}
                      </span>
                  ...
                  <input
                    ...
                    value={m.progress}
                    onChange={(e) =>
                      updateMilestoneProgress(m.id, parseInt(e.target.value))
                    }
          ...
          <div className="card p-5">
            ...
            <div className="space-y-2">
              ...
              {goal.actionItems?.map((item) => (
                <div
                  key={item.id}
                  className="flex items-start gap-2 p-2 rounded-lg hover:bg-zinc-800 transition-colors"
                >
                  <div
                  ...
                  <div className="min-w-0">
                    <p
                      className={`text-sm ${item.status === "DONE" ? "text-zinc-600 line-through" : "text-zinc-300"} truncate`}
                    >
                      {item.title}
                    </p>
                    {item.assignee && (
