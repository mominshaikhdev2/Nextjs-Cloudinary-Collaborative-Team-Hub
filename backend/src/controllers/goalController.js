...
const createGoal = async (req, res) =>
{
  ...
  await logActivity({
    ...
    workspaceId, userId: req.user.id, metadata: { title: goal.title },
  });

  try { getIO().to(`workspace:${workspaceId}`).emit('goal:created', goal); } catch (_) { }
...
const updateGoal = async (req, res) =>
{
  ...
  await logActivity({
    action: 'UPDATED', entityType: 'GOAL', entityId: goalId,
    workspaceId, userId: req.user.id, metadata: { title: goal.title, status: goal.status },
  });

  try { getIO().to(`workspace:${workspaceId}`).emit('goal:updated', goal); } catch (_) { }
...
const deleteGoal = async (req, res) =>
{
  ...
  await logActivity({
    action: 'DELETED', entityType: 'GOAL', entityId: goalId,
    workspaceId, userId: req.user.id, metadata: { title: goal.title },
  });

  try { getIO().to(`workspace:${workspaceId}`).emit('goal:deleted', { id: goalId }); } catch (_) { }
...
const addProgressUpdate = async (req, res) =>
{
  ...
  await logActivity({
    action: 'PROGRESS_UPDATE', entityType: 'GOAL', entityId: goalId,
    workspaceId, userId: req.user.id, metadata: { preview: content.slice(0, 80) },
  });

  try { getIO().to(`workspace:${workspaceId}`).emit('goal:progress_update', { goalId, update }); } catch (_) { }
...
module.exports = {
  ...
  getProgressUpdates, addProgressUpdate,
};
